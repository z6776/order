...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'openid','username', 'nickname','level','gender','idcart_frontimage','vipendtime','levelname','idcart_backimage','barcodeimage','work_user_id', 'mobile', 'avatar','score',"approvestatus",'idcardno'];
     ...
     * @return boolean
     */
    public function registerwx($nickname,$openid,$avatar='', $sex = '',$extend = [])
    {
        // 检测openid是否存在
        if (!isset($openid) || empty($openid)) {
            $this->setError('openid 为空');
            return false;
        }
        $ip = request()->ip();
        $time = time();
        ...
        $params = array_merge($data, [
            'nickname'  => preg_match("/^1[3-9]{1}\d{9}$/", $nickname) ? substr_replace($nickname, '****', 3, 4) : $nickname,
            'salt'      => Random::alnum(),
            'jointime'  => $time,
            'joinip'    => $ip,
            'logintime' => $time,
        ...
        try {
            ...
            $this->_user = User::get($user->id);
            //设置Token
            $newToken = new \app\common\model\Token();
            $t = [
                "user_id"=>$user->id,
                "token_type"=>\app\common\model\Token::token_type_w,
                "token"=>Random::uuid(),
                "expires_at"=> date("Y-m-d H:i:s",time()+ $newToken::expires_at),
                "created_at"=>date("Y-m-d H:i:s",time())
            ];
           \app\common\model\Token::create($t,true);
           $this->_token = $t["token"];
            //设置登录状态
            $this->_logined = true;
