	...
	<view class="content" v-if="loading">
		...
		<view class="introduce-section m-t-20">
			<view class="price-box">
				...
				<text class="m-r-auto fz-FF2525 fz-32 fz-weight">{{info.price || ""}}</text>
				<text class="fz-28 basecolor fz-weight">已售{{info.sales || "0"}}件</text>
			</view>
			<text class="title">{{info.name}}</text>
			<view class="bot-row" v-if="info.shortdesc">
				{{ info.shortdesc || "" }}
			</view>
		</view>
		<!-- 活动商品不要规格 -->
		...
		<view class="eva-section" v-if="evaluate && evaluate.length">
			<view class="e-header" @click="$util.navTo('/pages2/evaluate/index?type=yl&product_id='+info.id)">
				<text class="m-r-auto fz-32 fz-000000 fz-bold">服务评价</text>
			...
			<view class="eva-box" v-for="(item, index) in evaluate" :key="item.id">
				...
				<view class="right">
					...
					<view class="m-t-20">
						 <image v-for="(v,j) in $util.toarr(item.pjimages)" :key="j"  @click.stop="$util.lookimgarr(j,item.pjimages)" :src="$util.getImgurl(v)" mode="aspectFill" class="wh140"></image>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="detail-desc">
			...
			<view class="p-l-20 p-r-20">
				<rich-text :nodes="info.briefcontent | filterdetail"></rich-text>
			</view>
		</view>
		<!-- 底部操作菜单 -->
...
<script>
	...
	export default {
		...
		computed:{
			...
			listfn(){
			    let imgs = this.info?.groupimages;
				return imgs ? this.info.groupimages.split(",") : [];
			}
		},
		async onShareAppMessage(e){
			console.log(this.params,"我的转发俩即可");
			try{
				uni.showLoading({
					title:"分享图片生成中...",
					mask:true
				...
				uni.hideLoading();
				return {
					path:`/pages/careservices/detail?${this.objectToQueryString(this.params)}`,
					imageUrl:imageurl,
		...
		methods: {
			...
			yyfn(){
				...
				const { id,name,shortdesc,coverimage,price } = this.info;
				this.$u.route({
					url: '/pages3/booknow/index',
					params: { id,name,shortdesc,coverimage,price }
				})
			},
			// 模仿php
			...
			// 收藏商品
			async collectShop(){
				  await this.$u.api.addCollect({product_id:this.params.id,type:2});
			...
			async getDetail() {
				uni.showLoading({
                 ...
                 const res = await this.$u.api.getYanglaoInfo({
