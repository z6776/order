		...
		<view class="uni-stat-box" :class="{'uni-stat__actived': current}">
			<view class="uni-select" :style="{height:multiple?'100%':' 35px'}"
				...
				<view class="uni-select__selector"
					...
					<scroll-view scroll-y class="uni-select__selector-scroll" @scrolltolower="scrolltolower">
						<view class="uni-select__selector-empty" v-if="filterMixinDatacomResData.length === 0">
							<span>{{emptyTips}}</span>
						</view>
						<view v-else :class="['uni-select__selector-item', {'uni-select_selector-item_active' :multiple && currentArr.includes(item[dataValue])}]"
							style="display: flex;justify-content: space-between;align-items: center;"
							v-for="(item,index) in filterMixinDatacomResData" :key="index" @click="change(item)">
							<span :class="{'uni-select__selector__disabled': item.disable}">{{formatItemName(item)}}</span>
							<uni-icons v-if="multiple&&currentArr.includes(item[dataValue])" type="checkmarkempty" color="#007aff" />
						</view>
					</scroll-view>
					<u-divider v-if="isEnd && !(filterMixinDatacomResData.length === 0)">没有更多了</u-divider>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			initDefVal() {
				...
				// 
				if (this.multiple && !this.filterInput){
					// const mixinDatacomResData = JSON.parse(JSON.stringify(this.mixinDatacomResData)) || []
					const mixinDatacomResData = JSON.parse(JSON.stringify(this.total_list)) || []
					if (!defValue) defValue = [];	
					let vArr = [];
					for (let i = 0; i < defValue.length; i++) {
						let item = defValue[i];
						for (let j = 0; j < mixinDatacomResData.length; j++) {
						    let element = mixinDatacomResData[j];
							if(element[this.dataValue]==item){
								vArr.push(element);
							}
						}
					};
			...
			emit(val) {
				if (this.multiple) {
					...
					const currentArr = this.mixinDatacomResData.filter(item => this.currentArr.includes(item[this.dataValue]))
			...
			toggleSelector() {
				...
				// 关闭且没有数据时初始化列表
					// debugger;
				if(!this.showSelector && this.filterMixinDatacomResData && this.filterMixinDatacomResData.length === 0){
					this.$emit("close");
				}
				if(this.multiple){
					this.$emit("close");
